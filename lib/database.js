...
var addFriend = function (username, friendName, callback) {
	findUser(username, function (err, user) {
		...
		findUser(friendName, function (err, friend) {
			if(err) return callback("Not found", null);
			User.findOneAndUpdate({userId: username}, {$push: {friendList: {friendId: friendName}}}, function(err, data){
...
var removeFriend = function (username, friendName, callback) {
	findUser(username, function (err, user) {
		if(err) return callback("Not found", null);
		findUser(friendName, function (err, friend) {
			if(err) return callback("Not found", null);
			User.findOneAndUpdate({userId: username}, {$pull: {friendList: {friendId: friendName}}}, function(err, data){
  				if(err) return callback(err, null);
				callback(null, friendName);
			});
		});
